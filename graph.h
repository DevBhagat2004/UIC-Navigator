...
class graph {
 private:
  // Adjacency list: maps each vertex to a map of its neighbors and edge weights
 ...
 public:
  ...
  bool addEdge(VertexT from, VertexT to, WeightT weight) {
    ...
    bool isNewEdge = !adjList[from].count(to);
  ...
  bool getWeight(VertexT from, VertexT to, WeightT& weight) const {
    ...
    auto it = adjList.at(from).find(to);
  ...
  set<VertexT> neighbors(VertexT v) const {
    ...
    if (!adjList.count(v)) {
      return S;  // Return empty set if vertex doesn't exist
    }
    
    for (const auto& [neighbor, weight] : adjList.at(v)) {
      S.insert(neighbor);
    }
